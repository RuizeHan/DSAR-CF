...
% clear, clc, 
% close all
function demo(inputpath,savepath,savepath_SF,savepath_GS)
...
SRC = inputpath;%'Data\SRC';       %Path of input images
BDCON = 'Data\BDCON';   %Path for saving bdCon feature image
RES =savepath; %'Data\Res';       %Path for saving saliency maps
...
for k=1:length(files)
    disp(k);
    exsavename=[savepath '\' files(k).name(1:length(files(k).name)-4) '_wCtr.png'];
 if(~exist(exsavename,'file'))
  try
...
%     SaveSaliencyMap(stage1, pixelList, frameRecord, smapName, true);
      ...
      errorimage=[inputpath '\' files(k).name];
      copyfile(errorimage,[errorfile files(k).name]);
  end
 end
end
